...
const fail=e=>{document.getElementById('error').style.display='block';document.getElementById('error').textContent=String(e);document.getElementById('loading').style.display='none';status.textContent='Error';console.error(e);};
 try{
  let B=await (await fetch('/bundle.json')).json();
  let c=B.config,v=c.viewer,R=c.planet.radius_metres/1000;
  ...
  let centre,east,north,span=v.view_width_metres/1000,overlay=0;
  const OVERLAYS=['Satellite','Height','Land mask','Temperature','Rainfall','Ruggedness','Strain','Ocean age','Crust','Rivers','Plates'];
 ...
 const blit=program(B,`#version 300 es
 precision highp float;uniform sampler2D uColor;uniform vec2 uOutputSize;out vec4 outColor;void main(){outColor=texture(uColor,gl_FragCoord.xy/uOutputSize);}`);
  ...
  let height=null,display=null,data={};
  async function loadTex(B,data,name){
   let bytes=null,dims=B.meta[name];
   try{bytes=await(await fetch('/data/'+name+'.bin')).arrayBuffer();}catch(e){bytes=null;}
   if(!bytes||!dims){
    // Backwards compat: caches built before a map get neutral guidance.
    const tex=gl.createTexture();gl.bindTexture(gl.TEXTURE_2D,tex);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.NEAREST);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,gl.NEAREST);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_S,gl.CLAMP_TO_EDGE);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_T,gl.CLAMP_TO_EDGE);
    if(name==='plates')gl.texImage2D(gl.TEXTURE_2D,0,gl.R32UI,1,1,0,gl.RED_INTEGER,gl.UNSIGNED_INT,new Uint32Array([0]));
    else gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA32F,1,1,0,gl.RGBA,gl.FLOAT,new Float32Array(name==='flow'?[1e3,0,0,0]:[0,0,0,0]));data[name]=tex;return;
   }
   const uint=name.startsWith('river_')||name==='plates',channels=name==='river_header'?2:name==='river_index'?1:4;
   const tex=gl.createTexture();gl.bindTexture(gl.TEXTURE_2D,tex);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.NEAREST);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,gl.NEAREST);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_S,gl.CLAMP_TO_EDGE);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_T,gl.CLAMP_TO_EDGE);
  ...
  for(const name of ['macro','climate','water','flow','plates','rivers','river_header','river_index','tectonics','crust'])await loadTex(B,data,name);
  height=program(B,B.height);display=program(B,B.display);
  async function buildGL(B){
   const nd={};
   for(const name of ['macro','climate','water','flow','plates','rivers','river_header','river_index','tectonics','crust'])await loadTex(B,nd,name);
   return{height:program(B,B.height),display:program(B,B.display),data:nd};
  }
  async function reloadWorld(){
   const NB=await (await fetch('/bundle.json')).json();
   if(NB.config.planet.radius_metres!==B.config.planet.radius_metres){location.reload();return;}
   const built=await buildGL(NB);
   for(const t of Object.values(data))gl.deleteTexture(t);
   gl.deleteProgram(height.p);gl.deleteProgram(display.p);
   height=built.height;display=built.display;data=built.data;B=NB;c=B.config;v=c.viewer;
   document.querySelector('#brand small').textContent=`${B.meta.tectonic_report.duration_myr.toLocaleString()} Myr · ${B.meta.tectonic_report.final_plates} plates · ${(R).toLocaleString()} km radius`;
   request(false);
  }
  function setOverlay(i){overlay=Math.max(0,Math.min(OVERLAYS.length-1,i|0));document.querySelectorAll('#overlays button').forEach((el,j)=>el.classList.toggle('active',j===overlay));request(false);}
  function bind(p,name,tex,unit){gl.activeTexture(gl.TEXTURE0+unit);gl.bindTexture(gl.TEXTURE_2D,tex);if(p.u[name])gl.uniform1i(p.u[name],unit);}
  function maps(p){for(const [i,kv] of Object.entries([['uMacro','macro'],['uClimate','climate'],['uWater','water'],['uFlow','flow'],['uRivers','rivers'],['uRiverHeader','river_header'],['uRiverIndex','river_index']]))bind(p,kv[0],data[kv[1]],+i);bind(p,'uTectonics',data.tectonics,9);bind(p,'uCrust',data.crust,10);bind(p,'uPlates',data.plates,12);}
 function uniforms(p,w,h,pixel,visibleSpan,bank){gl.uniform3fv(p.u.uCentre,centre);gl.uniform3fv(p.u.uEast,east);gl.uniform3fv(p.u.uNorth,north);gl.uniform2f(p.u.uSize,w,h);gl.uniform1f(p.u.uPixel,pixel);gl.uniform1f(p.u.uSpan,visibleSpan);
  const cell=new Int32Array(bank.length*3),fract=new Float32Array(cell.length);
  bank.forEach((a,i)=>{for(let j=0;j<3;j++){let q=centre[j]*R/a.scale+a.offset[j],k=Math.floor(q);cell[3*i+j]=k|0;fract[3*i+j]=q-k;}});
 ...
 function render(){pending=false;try{
  ...
  resize(field,sw+4,sh+4);gl.bindFramebuffer(gl.FRAMEBUFFER,field.fbo);gl.viewport(0,0,field.w,field.h);gl.useProgram(height.p);maps(height);uniforms(height,field.w,field.h,pixel,span+4*pixel,B.bank);gl.uniform1i(height.u.uExact,0);gl.drawArrays(gl.TRIANGLES,0,3);
  resize(colour,sw,sh);gl.bindFramebuffer(gl.FRAMEBUFFER,colour.fbo);gl.viewport(0,0,sw,sh);gl.useProgram(display.p);maps(display);uniforms(display,sw,sh,pixel,span,B.bank);
  bind(display,'uGround',field.textures[0],7);bind(display,'uEnvironment',field.textures[1],8);bind(display,'uWet',field.textures[2],11);gl.uniform1f(display.u.uBorder,border);if(display.u.uOverlay)gl.uniform1i(display.u.uOverlay,overlay);gl.drawArrays(gl.TRIANGLES,0,3);
 ...
 canvas.addEventListener('wheel',e=>{e.preventDefault();zoom(Math.exp(Math.max(-1,Math.min(1,e.deltaY*(e.deltaMode===1?16:1)*.0015))),e.clientX,e.clientY);},{passive:false});
  ...
  addEventListener('keydown',e=>{if(e.target&&/INPUT|TEXTAREA|SELECT/.test(e.target.tagName))return;const n=parseInt(e.key,10);if(!isNaN(n)&&n<OVERLAYS.length)setOverlay(n);});
  ...
  window.globe={ready:false,getView:()=>({...last}),setView:(lon,lat,widthKm)=>{orient(lon,lat);span=widthKm;request(false);},
   setOverlay:(i)=>setOverlay(i),getOverlay:()=>overlay,overlayNames:()=>[...OVERLAYS],reloadWorld:()=>reloadWorld(),
   setConfig:(patch)=>{Object.assign(v,((patch&&patch.viewer)||patch||{}));request(false);},
   read:()=>{gl.finish();gl.bindFramebuffer(gl.FRAMEBUFFER,field.fbo);gl.readBuffer(gl.COLOR_ATTACHMENT0);let a=new Float32Array(field.w*field.h*4);gl.readPixels(0,0,field.w,field.h,gl.RGBA,gl.FLOAT,a);gl.bindFramebuffer(gl.FRAMEBUFFER,null);return{w:field.w,h:field.h,values:a};},gl,get bundle(){return B;}};
